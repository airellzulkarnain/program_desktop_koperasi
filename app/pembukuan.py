...
def pembukuan(parent: ttk.Notebook):
    ...
    ttk.Label(sub_frame_1, text='Biaya - Biaya', font=('Arial', 14, 'bold')).grid(column=1, row=1, sticky=tk.NSEW)
    ttk.Label(sub_frame_2, text='Keterangan', font=('Arial', 12, 'normal')).grid(column=1, row=1, sticky=tk.EW, padx=4, pady=6)
    ttk.Label(sub_frame_2, text='Nominal', font=('Arial', 12, 'normal')).grid(column=3, row=1, sticky=tk.EW, padx=4, pady=6)
    ttk.Label(sub_frame_3, text='Pembukuan', font=('Arial', 14, 'bold')).grid(column=1, row=1, sticky=tk.NSEW)
    ttk.Button(sub_frame_3, text='Refresh...', command=lambda: refresh_pembukuan()).grid(column=2, row=1, sticky=tk.NSEW, padx=4, pady=6)
    ...
    ttk.Radiobutton(sub_frame_2, text='Debit', value='debit', variable=debit_kredit).grid(column=5, row=1, sticky=tk.EW, padx=4, pady=6)
    ttk.Radiobutton(sub_frame_2, text='Kredit', value='kredit', variable=debit_kredit).grid(column=6, row=1, sticky=tk.EW, padx=4, pady=6)
    biaya_button = ttk.Button(sub_frame_2, text='Tambah', style='green.TButton', command=lambda: biaya_klik())
    biaya_button.grid(column=7, row=1, sticky=tk.EW, padx=4, pady=6)

    pembukuan_treeview = ttk.Treeview(sub_frame_4, columns=('tanggal', 'uraian', 'debit', 'kredit', 'saldo'), show='headings')
    ...
    pembukuan_scrollbar = ttk.Scrollbar(sub_frame_4, orient=tk.VERTICAL, command=pembukuan_treeview.yview)
    ...
    cari_pembukuan_button = ttk.Button(sub_frame_5, text='Cari', style='blue.TButton', command=lambda: cari_pembukuan_klik())
    ...
    ttk.Radiobutton(sub_frame_6, text='Bulan', variable=range_pembukuan, value='0').grid(column=1, row=1, sticky=tk.NSEW, padx=4, pady=6)
    ttk.Radiobutton(sub_frame_6, text='Tahun', variable=range_pembukuan, value='1').grid(column=2, row=1, sticky=tk.NSEW, padx=4, pady=6)
    ...
    buat_laporan_button = ttk.Button(sub_frame_6, text='Buat Laporan', style='blue.TButton', command=lambda: buat_laporan_klik())
    ...
    def refresh_pembukuan():
        ...
        for pembukuan in himpunan_pembukuan:
            pembukuan_treeview.insert('', 'end', values=(pembukuan.tanggal.date(), pembukuan.uraian, 'Rp. '+crud.angka_mudah_dibaca(pembukuan.debit), 'Rp. '+crud.angka_mudah_dibaca(pembukuan.kredit), 'Rp. '+crud.angka_mudah_dibaca(pembukuan.saldo)))
        

    def biaya_klik():
        if messagebox.askokcancel('Warning !', 'Pastikan seluruh data sudah lengkap dan benar ! Setelah ini data tidak dapat dihapus dari pembukuan !', icon=messagebox.WARNING):
            if len(keterangan_biaya_entry.get()) > 0 and int(nominal_biaya_spinbox.get()) > 0 and len(debit_kredit.get()) > 0:
                if debit_kredit.get() == 'debit':
                    biaya_tambahan(keterangan_biaya_entry.get(), debit=int(nominal_biaya_spinbox.get()))
                elif debit_kredit.get() == 'kredit':
                    biaya_tambahan(keterangan_biaya_entry.get(), kredit=int(nominal_biaya_spinbox.get()))
    ...
    def buat_laporan_klik():
        ...
        main.geometry(f'300x200+{parent.winfo_screenwidth()//2-150}+{parent.winfo_screenheight()//2-100}')
        ...
        if range_pembukuan.get() == '0':
            def buat():
                if int(bulan_spinbox.get('')) > 0 and int(tahun_spinbox.get()) > 0: 
                    buat_laporan(int(range_pembukuan.get()), nama_sekolah_entry.get(), filedialog.askdirectory(title='Pilih Folder'), bulan=int(bulan_spinbox.get()), tahun=int(tahun_spinbox.get()))
                    dismiss()
            nama_sekolah_entry = ttk.Entry(main_frame)
            ttk.Label(main_frame, text='Bulan').grid(column=1, row=1, sticky=tk.NSEW, padx=4, pady=6)
            ttk.Label(main_frame, text='Tahun').grid(column=2, row=1, sticky=tk.NSEW, padx=4, pady=6)
            bulan_spinbox = ttk.Spinbox(main_frame, from_=1, to=12)
            tahun_spinbox = ttk.Spinbox(main_frame, from_=2022, to=2100)
            ttk.Label(main_frame, text='Instansi: ').grid(column=1, columnspan=2, row=3, sticky=tk.NSEW, padx=4, pady=6)
            nama_sekolah_entry.grid(column=1, columnspan=2, row=4, sticky=tk.NSEW, padx=4, pady=6)
            ...
            buat_button = ttk.Button(main_frame, text='Buat', style='blue.TButton', command=lambda: buat())
            buat_button.grid(column=1, columnspan=2, row=5, sticky=tk.NSEW, padx=4, pady=6)
            main_frame.columnconfigure(1, weight=1)
            ...
            def buat():
                if int(tahun_spinbox.get()) > 0: 
                    buat_laporan(int(range_pembukuan.get()), nama_sekolah_entry.get(), filedialog.askdirectory(title='Pilih Folder'), tahun=int(tahun_spinbox.get()))
                    dismiss()
            nama_sekolah_entry = ttk.Entry(main_frame)
            ttk.Label(main_frame, text='Tahun').grid(column=1, row=1, sticky=tk.NSEW)
            tahun_spinbox = ttk.Spinbox(main_frame, from_=2022, to=2100)
            buat_button = ttk.Button(main_frame, text='Buat')
            ttk.Label(main_frame, text='Instansi: ').grid(column=1, row=3, sticky=tk.NSEW, padx=4, pady=6)
    ...
    def cari_pembukuan_klik():
        global himpunan_pembukuan
        pembukuan_treeview.delete(*pembukuan_treeview.get_children())
        for pembukuan in filter(lambda pembukuan: cari_pembukuan_entry.get().lower() in (str(pembukuan.tanggal)+pembukuan.uraian).lower(), himpunan_pembukuan):
            pembukuan_treeview.insert('', 'end', values=(pembukuan.tanggal.date(), pembukuan.uraian, 'Rp. '+crud.angka_mudah_dibaca(pembukuan.debit), 'Rp. '+crud.angka_mudah_dibaca(pembukuan.kredit), 'Rp. '+crud.angka_mudah_dibaca(pembukuan.saldo)))
